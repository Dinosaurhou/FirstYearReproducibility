...
if __name__ == '__main__':
    ...
    # --- 运行模拟 ---
    # 模拟不同初始攻击强度下的级联失效
    attack_fractions = np.linspace(0, 1, 21) # 从 0% 到 100% 的攻击强度
    final_sizes = []

    print("开始模拟级联失效...")
    for p_attack in attack_fractions:
        # 为了结果的稳定性，可以对每个攻击强度进行多次模拟并取平均值
        num_runs = 5
        avg_size = 0
        ...
        print(f"初始攻击比例 p = {p_attack:.2f}, 最终网络规模比例 = {avg_size:.3f}")

    # --- 结果可视化 ---
    plt.figure(figsize=(10, 6))
    plt.plot(attack_fractions, final_sizes, 'o-', label='相互依赖网络')
    plt.xlabel('初始攻击移除的节点比例 (1-p)')
    plt.ylabel('最终相互连接的巨型分量大小 (P∞)')
    plt.title('相互依赖网络的级联失效')
    plt.grid(True)
    
    # 论文中的一个关键发现是存在一个临界点
    # 在此点之后，网络会发生灾难性的崩溃（一阶相变）
    # 你可以在图上观察到这个现象
    
    # 作为对比，可以画出单个网络受攻击时的曲线
    # single_network_sizes = []
    # for p_attack in attack_fractions:
    #     net_A_single = G_A.copy()
    #     nodes_to_remove = np.random.choice(net_A_single.nodes(), size=int(N * p_attack), replace=False)
    #     net_A_single.remove_nodes_from(nodes_to_remove)
    #     giant = get_giant_component(net_A_single)
    #     single_network_sizes.append(len(giant.nodes()) / N)
        
    # plt.plot(attack_fractions, single_network_sizes, 's--', label='单个网络')
    plt.legend()
    plt.show()