...
if __name__ == '__main__':
    # --- 网络参数 ---
    N = 10000  # 节点数固定
    avg_degree_values = [2, 3, 4, 5] # 不同的平均度

    # --- 模拟参数 ---
    attack_fractions = np.linspace(0, 1, 51) # 攻击强度采样点
    num_runs = 5 # 每个点的模拟次数
    ...
    # --- 对每个平均度<k>值运行模拟 ---
    for avg_degree in avg_degree_values:
        print(f"\n开始模拟 N = {N}, <k> = {avg_degree} 的网络...")
        P = avg_degree / (N - 1)  # ER网络连接概率随<k>变化
        ...
        for p_attack in attack_fractions:
            ...
            # 为了简洁，可以注释掉下面的打印语句以加快运行速度
            # print(f"  <k>={avg_degree}, 初始攻击比例 p = {p_attack:.2f}, 最终网络规模比例 = {avg_size:.3f}")

        # --- 在图表中绘制当前<k>值的结果 ---
        plt.plot(attack_fractions, final_sizes, 'o-', label=f'<k> = {avg_degree}')

    # --- 结果可视化 ---
    plt.xlabel('初始攻击移除的节点比例 (1-p)')
    plt.ylabel('最终相互连接的巨型分量大小 (P∞)')
    plt.title(f'不同平均度的相互依赖网络级联失效 (N = {N})')
    plt.grid(True)
    plt.legend()
    plt.show()